...
class DQN(object):
    def __init__(self):  # 定义DQN的一系列属性
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, 730))  # 初始化记忆库，一行代表一个transition
        ...
        if self.load_models:
            ...
            checkpoint = torch.load("dqn/searchgoal_ros_test/model/" + str(self.load_ep) + "a.pt")
    ...
    def save_model(self, dir):
        ...
        torch.save(state, "/home/hhd/PycharmProjects/RL_study_/dqn/searchgoal_ros_test/model/" + dir + "a.pt")
    ...
    for e in range(10000):  # 400个episode循环
        ...
        for t in range(episode_step):  # 开始一个episode (每一个循环代表一步)
            ...
            if done:  # 如果done为True
                ...
                h, m = divmod(m, 60)
                rospy.loginfo('Ep: %d score: %.2f memory: %d epsilon: %.2f time: %d:%02d:%02d', e, episode_reward_sum, dqn.memory_counter,
                              dqn.epsilon, h, m, s)
