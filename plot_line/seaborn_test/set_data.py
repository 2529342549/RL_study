...
def get_data():
    ...
    for _, log_file in enumerate(args.log_files):
        ...
        for r in re.findall(train_pattern, log):
            rewardList.append(float(r[1]))

    # painter.deleteData('LSTM')
    painter.addData(rewardList, 'LSTM')
    painter.saveData('/home/hhd/PycharmProjects/RL_study_/plot_line/data/reward.csv')
    painter.setTitle('lstm reward')
