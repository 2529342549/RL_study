...
def trpo_step(model, get_loss, get_kl, max_kl, damping):
    ...
    def Fvp(v):
        ...
        grads = torch.autograd.grad(k1, model.parameters(), create_graph=True)
        flat__grad_grad_k1 = torch.cat([grad.contiguous().view(-1) for grad in grads])

        return flat__grad_grad_k1 + v * damping
