...
class train_sys:
    def __init__(self, max_step=200, goal_step=15, episodes=10000, train_step=4, save_step=3, ):
        self.episodes = episodes
        self.env = env_ppo.Env()
        self.env.reset()
        ...
        self.Agent = agent_ppo.PPO_Agent(state_shape=self.state_size, action_size=self.action_size)
        self.reward = 0
        self.end_count = 0
        self.target = 16
    ...
    def run(self):
        for epi in range(self.episodes):
            ...
            while not done:
                ...
                self.Agent.add_buffer(self.state, action_one_hot, self.reward, next_state, action_prob, done, epi)
...
#         self.action_size = self.env.action_size
#         self.action_space = self.env.action_space
#         self.Agent = agent_ppo.PPO_Agent(state_size=self.state_size, action_size=self.action_size, action_space=self.action_space)
#         self.reward = 0
#
#     def run(self):
#         done = False
